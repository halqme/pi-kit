...
test("separates skill reads from explicit skill commands", () => {
  const result = analyzeLines([
    JSON.stringify({
      ...
      message: {
        role: "user",
        content: [{ type: "text", text: "Use /skill:things for this task." }],
      },
    }),
    JSON.stringify({
      type: "message",
      message: {
        role: "assistant",
        content: [
          { type: "toolCall", name: "read", arguments: { path: "/repo/skills/things/SKILL.md" } },
          { type: "toolCall", name: "bash", arguments: { path: "/repo/skills/other/SKILL.md" } },
        ],
      },
    }),
  ]);
  assert.deepEqual(result.skills.things, { reads: 1, explicit: 1 });
  assert.equal(result.skills.other, undefined);
});
