...
- `locate`は編集意図に含まれる`symbols`または`terms`から、`scope`内の宣言ノードを決定論的に順位付けします。既定では上位3件（`maxCandidates`で1〜5件）を返します。完全一致symbolで明確に首位かつ本文が6,000 bytes以下なら`mode: "source"`としてその本文も返し、`next`でそのcontinuationを直接`replace`できます。同じ候補を改めて`inspect`する必要はありません。そうでなければ`mode: "cards"`としてシグネチャ、親宣言、呼出し・分岐・終了の概略、本文サイズ、continuationだけを返します。cardsでは選んだcontinuationのsourceを確認してから編集します。既知の編集対象または意図がある場合の既定actionです。
...
- `inspect_many`は同一ファイルの複数continuationを一度にsourceまで確認します。複数のcardsを編集することが既に決まっている場合に、個別の`inspect`往復をまとめます。成功時の`next`は対応する`replace_many`テンプレートです。
- `replace`はsourceが実際に返された有効な`continuation`と完全な`replacement`を受け、現在のsource hash、ノード型・範囲・親文脈を再検証してから保存します。`locate(mode: "source")`または`inspect(detail: "source")`を経ていないcontinuationは拒否します。増分再解析、構文エラー、原子的保存の検証は維持します。
- `replace_many`は同一ファイル内の複数continuationを全件検証し、各sourceが確認済みで置換後の構文検査に成功した場合だけatomicに保存します。1件でも未確認、stale、範囲重複、構文エラーがあればファイルを変更しません。
...
`locate`、`inspect`、`inspect_many`、`search`の結果には、後続操作に使えるcontinuationまたは`next`が含まれます。`locate`が`mode: "source"`なら通常はそのまま`replace`します。`mode: "cards"`では、構造カードから選んだ一件だけなら`inspect`し、同一ファイルの複数箇所を変更することが決まっているなら`inspect_many`でまとめてsourceを確認してから`replace_many`します。失敗時には、再検査などの回復に使える情報を返します。

旧`syntax_inspect`、`syntax_search`、`syntax_replace`は登録されません。更新後はPiで`/reload`を実行してください。

### フック

- `before_agent_start`: ツール選択前には対応ソースでのAstrolabe優先を短く案内し、選択後は編集意図・既知symbolには`locate`、例外的な調査には`search`/`inspect`を案内します。`locate(mode: "source")`では再inspectせず直接replaceし、複数の同一ファイルcardsでは`inspect_many`を優先します。未対応・生成物・設定・新規ファイルは通常のツールを使います。
...
Astrolabeの構文検査は型検査やLintの代替ではありません。編集後は、各言語の型検査・Lint・プロジェクトのテストを別途実行してください。ツール結果の`details.metrics`にはaction別回数、`locate`が返した候補数、要求・応答の概算トークン量を記録します。session-metrics側でも`astrolabe.locate`、`astrolabe.inspect`、`astrolabe.inspect_many`、`astrolabe.replace`、`astrolabe.replace_many`を別集計できるため、同じfixtureで読取りaction/書込みaction比とツール往復数を比較できます。
...
- `source_not_inspected`: `mode: "cards"`等でsourceをまだ取得していないcontinuationをreplaceしようとしました。`inspect`または`inspect_many`でsourceを確認してください。
