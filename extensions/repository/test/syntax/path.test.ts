...
test("resolves existing files using real paths", async () => {
  ...
  await assert.rejects(resolveExistingPath(dir, "../outside.ts"), /existing file|project root/);
});
...
test("rejects a symlink that resolves outside the project root", async () => {
  ...
  await assert.rejects(resolveExistingPath(dir, "link.ts"), /project root/);
});
...
test("treats the Git root as the path boundary when Pi starts in a subdirectory", async () => {
  const root = await mkdtemp(join(tmpdir(), "astrolabe-project-root-"));
  const nested = join(root, "extensions", "delegate");
  const sibling = join(root, "extensions", "repository", "sample.ts");
  await mkdir(nested, { recursive: true });
  await mkdir(join(root, "extensions", "repository"), { recursive: true });
  await writeFile(sibling, "export const answer = 42;\n");
  await exec("git", ["init"], { cwd: root });

  assert.equal(await resolveExistingPath(nested, "extensions/repository/sample.ts"), sibling);
  assert.equal(await resolveExistingPath(nested, sibling), sibling);
});
