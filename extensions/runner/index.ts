...
function taskFor(state: PlanState): string {
  ...
  if (!remaining.length)
    return [
      ...
      "Verify the requested outcome against the actual workspace and available evidence.",
      "Do not infer completion from the empty TODO list alone.",
      "Call runner.finish with a concise evidence summary only when the protocol is satisfied; otherwise continue working or call runner.stop if blocked.",
    ].join("\n");
...
export default function runnerExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Treat completed TODO steps as execution evidence, not as a completion verdict. After every step is reported complete, verify the requested outcome and call runner.finish with concise evidence.",
          ...
          content: [
            {
              ...
              text: verificationPending
                ? "All TODO steps are reported complete. Verify the requested outcome, then call runner.finish with concise evidence."
                : "Progress recorded.",
            },
          ],
          details: state,
