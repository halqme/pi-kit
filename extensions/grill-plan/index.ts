...
export default function grillPlanExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (params.goal?.trim()) {
        pi.sendUserMessage(params.goal.trim(), { deliverAs: "followUp" });
      }
      return {
  ...
  pi.on("before_agent_start", async (event) => {
    if (phase === "planning" || phase === "ready") {
      const stateInstruction =
        phase === "ready"
          ? "A complete plan is awaiting explicit approval. Do not implement it. If the user requests changes, revise the plan while remaining read-only."
