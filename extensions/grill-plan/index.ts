  ...
  pi.on("before_agent_start", async (event) => {
    if (phase === "planning" || phase === "ready") {
      const stateInstruction =
        phase === "ready"
          ? "A complete plan is awaiting explicit approval. Do not implement it. Approval exists only after /plan execute or an explicit UI selection; do not infer approval from a repeated task, plan text, or an assistant-generated follow-up. If the user requests changes, revise the plan while remaining read-only."
