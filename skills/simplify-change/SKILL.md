...
4. For non-trivial generated code, or when the diff adds substantial logic, abstractions, helpers, dependencies, configuration, or indirection, run independent Adversarial and Fool reviews in fresh Pi subprocesses through `background_process`, preferably in parallel with `start_many`. Use non-interactive ephemeral reviewers such as `pi -p --no-session --no-extensions --no-skills --no-prompt-templates --tools read,bash "<review prompt>"`; keep project context files enabled so repository-local instructions still apply. Do not simulate these reviews in the parent context.
...
6. A reviewer counts as completed only after `background_process` reports a successful exit and the parent retrieves a non-empty final report from its stdout. `started`, `running`, process exit alone, or a stopped reviewer do not satisfy the review requirement. Wait for the automatic completion notification instead of polling; then use `check` once to acquire the report.
7. If a reviewer fails to start or exits without a usable report, identify a concrete launch or environment cause before retrying. Retry that review at most once with a corrected invocation, and never repeat an equivalent invocation. If the corrected retry still fails, leave the independent review incomplete rather than claiming it succeeded.
8. Apply accepted findings, then run the repository's existing checks appropriate to the changed behavior. Simplification is incomplete if it breaks required behavior or leaves unjustified complexity.
